...
class TranslatorService implements TranslatorInterface
{
        ...
        foreach ($object::getTranslatablePropertyNames() as $translatablePropertyName) {
            ...
            if (method_exists($object, $getter)) {
                ...
                if (is_string($propertyValue)) {
                    $translated[$translatablePropertyName] = $this->adapter->translate($propertyValue, $toLanguage, $fromLanguage);
                    continue;
     ...
     * @throws TranslationException
     */
    private function iterateHelper(array $array, string $toLanguage, string $fromLanguage = TranslatorInterface::DEFAULT_LANG): array
    {
        $result = [];
